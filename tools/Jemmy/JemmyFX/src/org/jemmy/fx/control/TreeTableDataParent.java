 ...
 */
class TreeTableDataParent<AUX> extends ItemDataParent<TreeItem, AUX> {

    TreeItem<? extends AUX> root;
    TreeTableViewWrap<? extends TreeTableView> treeTableViewWrap;

    TreeTableDataParent(TreeTableViewWrap<? extends TreeTableView> treeTableViewWrap, TreeItem<? extends AUX> root, Class<AUX> type) {
        super(treeTableViewWrap, type);
        this.root = root;
        this.treeTableViewWrap = treeTableViewWrap;
    }

    TreeTableDataParent(TreeTableViewWrap<? extends TreeTableView> treeViewWrap, Class<AUX> type) {
        this(treeViewWrap, null, type);
    }

    @Override
    protected <DT extends AUX> Wrap<? extends DT> wrap(Class<DT> type, TreeItem item, AUX aux) {
        return new TreeTableDataWrap<DT>(type, item, treeTableViewWrap, getEditor());
    }
