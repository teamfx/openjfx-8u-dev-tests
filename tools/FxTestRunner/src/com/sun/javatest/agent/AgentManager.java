...
/**
 * Access to the facilities provided by JavaTest Harness agents.
...
public class AgentManager {
    ...
    public static interface Observer {
         ...
         */
        void started(Connection connection, String tag, String request,
                String executable, String[] args, boolean localizeArgs);
    ...
    private synchronized void notifyStarted(Connection connection, String tag,
            String request, String executable, String[] args,
            boolean localizeArgs) {
        for (int i = 0; i < observers.length; i++) {
            observers[i].started(connection, tag, request, executable, args,
                    localizeArgs);
        }
    }

    private synchronized void notifyFinished(Connection connection,
            Status status) {
        for (int i = 0; i < observers.length; i++) {
     ...
     * @throws IOException if a problem occurs establishing the connection
     */
    public Task connectToActiveAgent() throws ActiveAgentPool.NoAgentException,
    ...
    public class Task {
        ...
        Task(Connection c) throws IOException {
            ...
            out = new DataOutputStream(new BufferedOutputStream(c
                    .getOutputStream()));
        }
        ...
        public Status executeTest(String tag, String className, String[] args,
                boolean localizeArgs, PrintWriter log, PrintWriter ref) {
            return run(tag, "executeTest", className, args, localizeArgs, log,
                    ref);
        }
         ...
         */
        public Status executeCommand(String tag, String className,
                String[] args, boolean localizeArgs, PrintWriter log,
                PrintWriter ref) {
            return run(tag, "executeCommand", className, args, localizeArgs,
                    log, ref);
        }
        ...
        public Status executeMain(String tag, String className, String[] args,
                boolean localizeArgs, PrintWriter log, PrintWriter ref) {
            return run(tag, "executeMain", className, args, localizeArgs, log,
                    ref);
        }

        private Status run(String tag, String request, String executable,
                String[] args, boolean localizeArgs, PrintWriter log,
                PrintWriter ref) {
            notifyStarted(connection, tag, request, executable, args,
                    localizeArgs);
            Status result = null;
            try {
                ...
                if (e instanceof InterruptedIOException)
                    result = Status
                            .error("Communication with agent interrupted! (timed out?)");
                else
                    result = Status.error("Problem communicating with agent: "
                            + e);
            }
            finally {
            ...
            while (status == null) {
                ...
                case Agent.STATUS:
                    ...
                    case Status.ERROR:
                        ...
                        break;
                    default:
                        status = Status.failed("Bad status from test: type="
                                + type + " reason=" + reason);
                        break;
                    }
                    break;
        ...
        private void sendClasses() throws IOException {
            if (classPath != null) {
                for (int i = 0; i < classPath.length; i++) {
                    if (classPath[i].isDirectory()) {
                        String rootDirPath = classPath[i].getCanonicalPath();
                        if(!rootDirPath.endsWith(File.separator)) {
                            rootDirPath += File.separator;
                            doSendClasses(rootDirPath, classPath[i]);
                        }
                    }else{
                        String relPath = classPath[i].getName();
                        sendFile(classPath[i], relPath);
                    }
                }
            }
        }
        private void doSendClasses(String rootDirPath, File currDir ) throws IOException {
            File [] files = currDir.listFiles();
            for(int fc = 0; fc < files.length; fc ++) {
                String name = files[fc].getName();
                if(files[fc].isDirectory()) {
                    doSendClasses(rootDirPath, files[fc]);
                }
                else /*if(name.endsWith(".class"))*/ {
                    String filePath = files[fc].getCanonicalPath();
                    String relPath = filePath.substring(rootDirPath.length());
                    sendFile(files[fc], relPath);
                }
            }
        }

        private void sendFile(File f, String relPath) throws IOException {
            //XXX
            if(f.getName().equals("javatest.jar") || f.getName().equals("jfxrt.jar")){
                return;
            }

            System.out.println(">>>Send file: " + relPath);

            out.write(1);
            out.writeUTF(relPath);
            byte[] data = read(new FileInputStream(f), ((int) f.length()));
            if (data == null) {
                out.writeInt(0);
            } else {
        ...
        private File[] split(String s) {
            ...
            Vector v = new Vector();
            int start = 0;
            for (int i = s.indexOf(pathCh); i != -1; i = s.indexOf(pathCh,
                    start)) {
