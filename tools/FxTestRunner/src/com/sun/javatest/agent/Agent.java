...
public class Agent implements Runnable {
    ...
    public interface Observer {
         ...
         */
        void execTest(Agent agent, Connection c, String tag, String className,
                String[] args);
         ...
         * @param args      Arguments for the class to be run.
         */
        void execCommand(Agent agent, Connection c, String tag,
         ...
         * @param args      Arguments for the class to be run.
         */
        void execMain(Agent agent, Connection c, String tag, String className,
                String[] args);
    ...
    public Agent(ConnectionFactory connectionFactory, int concurrency) {
        if (concurrency <= 0)
            throw new IllegalArgumentException("bad concurrency: "
                    + concurrency);
     ...
     * used to remap file systems which might have diffeerent mount points
                ...
                while (threads.size() < maxThreads && !closing) {
                    Thread t = new Thread(new Runnable() {
                        public void run() {
                            ...
                            if (tracing)
                                traceOut.println("THREAD " + curr.getName()
                                        + " STARTED " + getClass().getName());
    ...
    private class Notifier {
        ...
        public synchronized void execTest(Connection cconnection, String tag,
                String className, String[] args) {
            for (int i = 0; i < observers.length; i++)
                observers[i].execTest(Agent.this, cconnection, tag, className,
                        args);
        }

        public synchronized void execCommand(Connection cconnection,
                String tag, String className, String[] args) {
            for (int i = 0; i < observers.length; i++)
                observers[i].execCommand(Agent.this, cconnection, tag,
                        className, args);
        }

        public synchronized void execMain(Connection connection, String tag,
                String className, String[] args) {
            for (int i = 0; i < observers.length; i++)
                observers[i].execMain(Agent.this, connection, tag, className,
                        args);
        }
    ...
    private synchronized void setSystemStreams(Object owner, PrintStream out,
            PrintStream err) throws InterruptedException, SecurityException {
        if (owner == null)
    ...
    private synchronized void resetSystemStreams(Object owner)
        ...
        if (owner != currSystemStreamOwner)
            throw new IllegalStateException("expected: " + owner + " found: "
                    + currSystemStreamOwner);
     ...
     */
    static public boolean tracing = false; // hmm, public??
    ...
    static final String productName = "JavaTest Harness Agent";

    static final String productVersion = "JTA_3.1.4";

    static final String productCopyright = "Copyright 1996-2007 Sun Microsystems, Inc";

    static final byte CLASS_FILES = (byte) 'c';

    private static int tmpCounter = -1;
...
//        }
        ...
        if(tmpCounter == -1) {
            tmpCounter = new Random().nextInt() & 0xffff;
        }
        tmpCounter ++;
        tmpDir += prefix + Integer.toString(tmpCounter);
        traceOut.println("DIR=" + tmpDir);
        File f = new File(tmpDir);
        f.mkdir();
        return f;

    }
    private static boolean createClassesDir(String relPath) {
        ...
        classesDir = new File(userDir, relPath);
        boolean b = classesDir.mkdir();
        traceOut.println("DIR=" + classesDir.getPath());
        return b;
    }
    private static void createWorkDir(String relPath) {
        if(workDir == null) {
            String userDir = System.getProperty("user.dir");
            if(!userDir.endsWith(File.separator)) {
                userDir += File.separator;
            }
            userDir +="FX" + File.separator + "SCRATCH" + File.separator;
    ...
    class Task {
        ...
        public void handleRequest() throws ConnectionFactory.Fault {
            ...
            try {
                ...
                if (pVer != protocolVersion)
                    throw new IOException("protocol mismatch;" + " expected "
                            + protocolVersion + " received " + pVer);
                ...
                if (request.equals("executeTest")
                        || request.equals("executeCommand")
                        || request.equals("executeMain"))
                    status = execute();
                else {
                    if (tracing)
                        traceOut.println("Unrecognized request for agent: `"
                                + request + "'");
                    status = Status.error("Unrecognized request for agent: `"
                            + request + "'");
                }
                ...
                */
                ...
                if (connection.isClosed())
                    ...
                    notifier.exception(connection, new IOException(
                            "timeout awaiting close from AgentManager"));
            }
            catch (InterruptedException e) {
                if (tracing) {
        ...
        private void receiveClasses(String pathName) throws IOException, ClassNotFoundException {
            if (!Boolean.getBoolean("javatest.agent.send")) {
                return;
            }

//          clearClassesDir();
            if (createClassesDir(pathName)) {
//                classesDir = createTempDir("fxClasses");
//                classesDir.deleteOnExit();

                out.write(CLASS_FILES);
                out.flush();
                while (true) {
                    int code = in.read();
                    if (code == 0) {
                    ...
                    if (tracing) {
                        traceOut.println("Class transfered: " + className);
                    }
                    className = className.replace('\\', File.separatorChar);
                    className = className.replace('/', File.separatorChar);

                    int nameIdx = className.lastIndexOf(File.separatorChar);
                    if (nameIdx != -1) {
                        File path = new File(classesDir, className.substring(0, nameIdx));
                        path.mkdirs();
                    }
        ...
        private void clearClassesDir() {
            if (classesDir != null) {
                doClearClassesDir(classesDir);
            }
            classesDir = null;
        }
        private void doClearClassesDir(File currentDir) {
            File [] files = currentDir.listFiles();
            for(int i = 0; i < files.length; i ++) {
                if(files[i].isDirectory()) {
                    doClearClassesDir(files[i]);
                }
                files[i].delete();
            }
            currentDir.delete();
        ...
        private Status execute() throws IOException {
            ...
            try {
                ...
                if (request.equals("executeTest")) {
                    ...
                    return Status.error("Unrecognized request for agent: `"
                            + request + "'");
            }
            catch (ClassCastException e) {
                if (tracing)
                    e.printStackTrace(traceOut);
                return Status.error("Can't execute class `" + className
                ...
                if (Boolean.getBoolean("javatest.agent.clear")) {
                    clearClassesDir();
                }
        ...
        private Status executeTest(Class c, String[] args, PrintWriter testLog,
                PrintWriter testRef) throws IOException,
        ...
        private Status executeCommand(Class c, String[] args,
                PrintWriter testLog, PrintWriter testRef, ClassLoader cl)
            ...
            if(c.getName().equals("com.sun.javatest.lib.FXProcessCommand")) {
                String tsName = args[0];
                modifiedArgs = new String[args.length - 1];
                System.arraycopy(args, 1, modifiedArgs, 0, args.length - 1);
                receiveClasses(tsName);
                if(classesDir != null) {
//                  modifiedArgs = new String[args.length + 2];
//                  modifiedArgs[0] = "-classPath";
//                  modifiedArgs[1] = classesDir.getCanonicalPath();
//                  System.arraycopy(args, 0, modifiedArgs, 2, args.length);

                    String cp = localCP(args);
                    for (int i = 0; i < args.length; i++) {
                        String s = args[i];
            ...
            return tc.run(modifiedArgs, testLog, testRef);
        }
        //XXX
        private String localCP(String[] args) throws IOException {
            String cp = "";
            for (int i = 0; i < args.length; i++) {
                String s = args[i];
                if (s.startsWith("-Djava.library.path=")) {
                    cp = s.replace("-Djava.library.path=", "").concat("/lib/jfxrt.jar");
                    break;
                }
            }
            cp += File.pathSeparatorChar + classesDir.getCanonicalPath();
            File[] ff = classesDir.listFiles();
            for (File f : ff) {
                if (f.getName().endsWith(".zip") || f.getName().endsWith(".jar")) {
                    cp += File.pathSeparator + f.getCanonicalPath();
                }
            }
            return cp;
        }

        private Status executeMain(Class c, String[] args, PrintWriter testLog,
                PrintWriter testRef) throws IOException,
                ClassNotFoundException, IllegalAccessException {
            notifier.execMain(connection, tag, c.getName(), args);

            PrintStream out = Deprecated.createPrintStream(new WriterStream(
                    testRef));
            PrintStream err = Deprecated.createPrintStream(new WriterStream(
                    testLog));
            try {
                ...
                return Status
                        .error("Can't find `public static void main(String[] args)' for `"
                                + c.getName() + "'");
            }
                ...
                return Status
                        .failed("interrupted while waiting for access to system streams");
            }
            finally {
         ...
         */
        synchronized void sendChars(byte type, char b[], int off, int len)
                throws IOException {
            out.write(type);
         ...
         */
        synchronized byte[] getClassData(String className)
                throws ClassNotFoundException {
            if (tracing) {
                traceOut.println("Loading remote class " + className);
            }
         ...
         * Get a resource
         */
        synchronized byte[] getResourceData(String resourceName)
                throws MissingResourceException, IOException {
            if (tracing) {
                traceOut.println("Loading remoute resource " + resourceName);
            }
            ...
            int size = in.readInt();
            if (size == 0)
                throw new MissingResourceException(resourceName, null,
                        resourceName);
            ...
            while (offset < data.length) {
                ...
                if (n == -1)
                    throw new IOException(resourceName
                            + ": EOF while reading resource data");
                else
                    offset += n;
            }
            ...
            if (agentClassLoader == null) {
                if (classLoaderConstructor == null) {
                    ...
                    try {
                        ...
                        classLoaderClass = Class.forName(pkg
                                + ".AgentClassLoader2");
                    }
                    catch (Throwable t) {
                    ...
                    try {
                        ...
                        classLoaderConstructor = classLoaderClass
                                .getDeclaredConstructor(constrArgTypes);
                    }
                    catch (NoSuchMethodException e) {
                        e.printStackTrace();
                    }
                ...
                try {
                    ...
                    return (ClassLoader) (classLoaderConstructor
                            .newInstance(args));
                }
                catch (InvocationTargetException e) {
                    Throwable t = e.getTargetException();
