...
public class SkinTest extends SkinTestBase {

    @Test(timeout = 300000)
    public void testGenerator() {
        for (Object nf : nodeChooser.as(Selectable.class).getStates()) {
            System.out.println("Generating for " + nf.toString());
            nodeChooser.as(Selectable.class).selector().select(nf);
            targetControl = parent.lookup(Node.class, new ByID<Node>(SELECTED_NODE_ID)).wrap();
        }
    }
}
