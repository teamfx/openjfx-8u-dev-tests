 ...
 */
public class MoveCaretChange extends Change {

    int pos;
    boolean select;

    public MoveCaretChange(int pos, boolean select) {
        this.pos = pos;
        this.select = select;
    }
    ...
    boolean canAppend(Change ch) {
        if(ch instanceof MoveCaretChange) {
            MoveCaretChange mch = (MoveCaretChange) ch;
            return !mch.select || (select == mch.select);
        } else {
            return false;
        }
    }

    void append(Change ch) throws ClassCastException {
        pos += ((MoveCaretChange)ch).pos;
        select = select && ((MoveCaretChange)ch).select;
    }
