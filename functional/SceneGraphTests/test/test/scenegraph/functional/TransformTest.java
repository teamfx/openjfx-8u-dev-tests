...
public class TransformTest extends TestBase {
    ...
    public void before() {
        super.before();
        rect = byID(scene, "the rectangle", Rectangle.class);
        labelX = byID(scene, "X", Text.class);
        labelY = byID(scene, "Y", Text.class);
        clear = byText(scene, "clear", Text.class);
        buttons = new ArrayList<Wrap<? extends Text>>();
        buttons.add(byText(scene, "scale", Text.class));
        buttons.add(byText(scene, "rotate", Text.class));
        buttons.add(byText(scene, "shear", Text.class));
        buttons.add(byText(scene, "translate", Text.class));
        buttonAffine = byText(scene, "affine", Text.class);
    ...
    public void debug() throws Exception {
        ...
        scene.mouse().click(1, (((rotate(point)))));
        checkLabels(p.x, p.y);
        clear.mouse().click();
        buttons.get(1).mouse().click();
        scene.mouse().click(1, ((rotate(scale(point)))));
        checkLabels(p.x, p.y);
        clear.mouse().click();
        buttons.get(2).mouse().click();
        scene.mouse().click(1, (scale(rotate(shear(point)))));
        checkLabels(p.x, p.y);
        clear.mouse().click();
        buttons.get(3).mouse().click();
        scene.mouse().click(1, scale(rotate(shear(translate(point)))));
        checkLabels(p.x, p.y);
